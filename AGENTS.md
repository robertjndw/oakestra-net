...
- **`env/`** — EnvironmentManager: owns host/namespace management only, creating/destroying network namespaces and dispatching between container (`env/ContainerNetDeployment.go`) and unikernel (`env/UnikernelNetDeployment.go`) deployments; delegates Service IP resolution to `resolver`
- **`resolver/`** — ServiceResolver: netlink-free packet-path resolution of Service IPs, Namespace IPs and Instance IPs against the translation table, kicking off background MQTT table queries on a miss; this is what lets `proxy` build and test without netlink. The `Resolver` interface is deliberately narrow - `GetInstanceIP` hands back a single `netip.Addr` rather than a whole `TableEntry`, because copying one per packet to read one address out of it used to be ~44% of the outgoing path. `TableGeneration` is the whole reason the steady-state packet path needs no table access at all: it is one atomic load, and an unchanged generation is what lets the flow cache answer on its own (see `proxy/proxyCache.go`)
- **`proxy/`** — split into `Datapath` (`proxy/Datapath.go`), which decides what happens to a packet (flow cache, fragment state, replay queues, resolves Service VIPs via `resolver` only when the flow cache can't answer) and performs no I/O, and `Tunnel` (`proxy/ProxyTunnel.go`), which owns the TUN device, the listen socket, the per-peer connection pool and its idle eviction, and the read loops that drive `Datapath.Handle` and act on the returned `Action`. I/O goes through two seams, `TunDevice` (`proxy/TunDevice.go`) and `TunnelSocket` (`proxy/TunnelSocket.go`), each with a `ReadBatch`/`WriteBatch` pair so a batch of packets costs one syscall instead of one per packet; both directions are batched today - ingoing (tunnel socket → TUN) writes the whole batch to the TUN device in one call, and outgoing (TUN → tunnel socket) groups one read's packets by destination (`outgoingBatch` in `proxy/ProxyTunnel.go`) and issues one write per distinct peer instead of one per packet, resolving that peer's `tunnelConn` once per group rather than once per packet. Each `tunnelConn` (a per-peer dialled, connected UDP socket - deliberately not unified with the listen socket, since that would swap tunnel traffic's source port from ephemeral to the fixed listen port) carries a family-specific batched writer, `*ipv4.PacketConn` or `*ipv6.PacketConn` from `golang.org/x/net`, chosen from the peer address's family since a node can have both v4 and v6 peers at once. `TunDevice` is backed by `golang.zx2c4.com/wireguard/tun` (not `github.com/songgao/water`, which this package no longer uses); uses `github.com/google/gopacket` for test fixtures only
...
- **`TableEntryCache/`** — in-memory cache for the service translation table. `TableManager` keeps three indexes rebuilt wholesale on every mutation: `byServiceIP`, `byNsIP`, and `byNsIPInstance`, the narrow one the packet path uses (namespace IP → instance addresses only). Every rebuild bumps an atomic `generation`; `Generation()` reads it without taking the lock, which is what the datapath's fast path is keyed on
- **`events/`** — internal pub/sub for table-query events (used to reset MQTT interest timeouts)
- **`clock/`** — a 1Hz coarse Unix-seconds clock, written by one ticker goroutine and read atomically by everyone else. Anything on the packet path that needs a "last used" timestamp reads this instead of calling `time.Now()`, which costs ~29ns per call and swamps the lookup it is timestamping. Because it only moves at 1Hz, the packet path's "last used" writes (`ConversionEntry.touch`, `events.Activity.Touch`) are guarded by a compare first: all but the first packet of each second would otherwise store the value already there, dirtying a cache line both packet loops read
...
The outgoing packet path is split by how warm the flow is. `ProxyCache.Lookup` answers from the flow cache alone whenever the route's `routeGen` still matches the table's current generation, so a steady-state packet touches neither the Service IP index nor the namespace IP index - it costs one atomic load, one shard mutex and one short bucket scan. Everything that consults the table lives in `Datapath.resolveRoute`, off that path. A generation bump sends the flow through `ProxyCache.Revalidate`, which *refreshes* the flow (its instance IPs and the destination job's activity stamp) rather than rerouting it: an established connection has to keep the replica it was pinned to. `FlowKey` therefore deliberately excludes the source instance IP - it is a function of the source namespace IP under a given generation, and including it would force a table lookup before the cache could even be asked.
