...
- **cluster-service-manager** pins `oakestra_messaging` as a git dependency in `cluster-service-manager/service-manager/requirements.txt`. Until the library lands in `oakestra/oakestra` the URL points at the `robertjndw/oakestra` fork on `@feat/add-pub-sub-interface`; re-pin to the org repo and a commit SHA or tag once it merges. To bump it, edit the ref in `requirements.txt` and reinstall (`pip install -r requirements-test.txt`).
- **node-net-manager** requires `github.com/oakestra/oakestra/libraries/oakestra_messaging_go` in `node-net-manager/go.mod`, redirected with a `replace` to the `robertjndw/oakestra` fork at a pseudo-version until the library lands upstream. To bump the commit: `go mod edit -replace github.com/oakestra/oakestra/libraries/oakestra_messaging_go=github.com/robertjndw/oakestra/libraries/oakestra_messaging_go@<sha> && go mod tidy`. After the upstream merge, drop the `replace`, tag the library `libraries/oakestra_messaging_go/vX.Y.Z` in oakestra and run `go get .../oakestra_messaging_go@vX.Y.Z`.
